      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid gap-8 md:grid-cols-3">
          {steps.map((step, index) => {
            ...
            return (
              <Card key={index} className="border-border bg-card relative overflow-hidden group hover:border-[#FF6B35]/50 transition-colors">
                <CardContent className="p-6">
                  <div className="flex items-center justify-between mb-4">
                    <div className="inline-flex h-12 w-12 items-center justify-center rounded-lg bg-[#FF6B35]/10">
                      <Icon className="h-6 w-6 text-[#FF6B35]" />
                    </div>
                    <span className="text-xs font-medium text-[#FF6B35] bg-[#FF6B35]/10 px-3 py-1 rounded-full">
                      {step.highlight}
                    </span>
                  </div>
